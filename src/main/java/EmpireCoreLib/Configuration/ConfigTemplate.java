...
public abstract class ConfigTemplate {
		...
		for (Field field : getClass().getDeclaredFields()) {
			if (field.getType().isAssignableFrom(ConfigProperty.class)) {
				try {
					...
					EmpireCoreLib.logger.info("Failed to access " + field.getName() + " while binding to config "
	...
	private void setProperty(ConfigProperty property, Property forgeProp) {
		try {
			...
			EmpireCoreLib.logger.info("Config value of " + property.name + " in category " + property.category
