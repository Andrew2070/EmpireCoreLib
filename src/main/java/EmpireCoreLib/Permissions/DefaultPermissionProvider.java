...
public class DefaultPermissionProvider implements IPermissionProvider {
	...
	public boolean checkPermission(PermissionContext context, String permission) {
		...
		if (PERM_SEED.equals(permission) && ! MinecraftLogicUtilities.getMinecraftServer().isDedicatedServer())
			return true;
	...
	protected int getOpLevel(GameProfile gameProfile) {
		...
		if (Ops.getPermissionLevel() == 0) {
			UserListOpsEntry entry = (UserListOpsEntry) MinecraftLogicUtilities.getMinecraftServer().getPlayerList().getOppedPlayers().getEntry(gameProfile);
